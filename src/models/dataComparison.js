...
class DataComparison {
  async index() {
    try {
      ...
      const citiesRef = db.collection('dataComparison');
      ...
      throw new Error(`Could not get dataComparison. Error: ${err}`)
    }
  }

  async show(name) {
    try {
      const cityRef = db.collection('dataComparison').doc(name);
      ...
      throw new Error(`Could not find dataComparison ${name}. Error: ${err}`)
    }
  }

  async create(b) {
    try {
      const res = await db.collection('dataComparison').doc(b.name).set(b);
  ...
  async update(b, all) {
    try {
      const dataRef = db.collection('dataComparison').doc(b.name);
